...
class TaskRepositoryH2(implicit ec: ExecutionContext)
  extends TaskEntity(TableQuery[TaskTable])
    with TaskRepository
{
  override def find(taskId: Int) = findById(taskId).map(_.headOption)
  ...
  override def length = h2DB.run(queryLength.length.result)
...
{
  def findById(id: Int): Future[Seq[Task]] = {
  ...
  def queryLength: Query[E, Task, Seq] = {
    for {
      task <- entity
    } yield task
  }
